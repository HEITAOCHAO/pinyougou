...
public class ItemCatServiceImpl implements ItemCatService {
	...
	public Map findOne(Long id){
		...
		TbTypeTemplate typeTemplat=typeTemplateMapper.selectByPrimaryKey(itemCat.getTypeId());
		Map map=new HashMap<>();
		
		Select2 select =new Select2(typeTemplat.getId().toString(),typeTemplat.getName());
		map.put("itemCat", itemCat);
		map.put("typeTemplat", select);
		return map;
	}
	...
	public void delete(Long[] ids) {
		for(Long id:ids){
			...
			if(itemCatMapper.selectByPrimaryKey(item.getParentId())!=null){
